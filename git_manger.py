...
class GitManagerApp(tk.Tk):
    ...
    def _github_auth_status(self) -> str:
        ...
        try:
            ...
            low = out.lower()
            if p.returncode == 0 and ('logged in' in low or 'account' in low):
                return "logged_in"
    ...
    def _git_push_status(self):
        ...
        if rc == 0 and out.strip():
            branch = out.strip()
        rc_f, out_f, err_f = run_git(['fetch', 'origin'])
        if rc_f != 0:
            err_detail = (err_f or out_f or '').strip()[:200]
            self._log('⚠️ fetch origin 失败: {}'.format(err_detail or '(无详细信息)'), 'WARN')
    ...
    def _push_via_proxy(self, branch_name):
        """通过 ghproxy.net + gh CLI token 代理推送，返回 True/False。"""
        gh_exe = self._find_gh_exe()
        tok_proc = subprocess.run([gh_exe, 'auth', 'token'],
                                 stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                 shell=False, text=True, encoding='utf-8', errors='replace')
        if tok_proc.returncode != 0:
            self._log('❌ gh auth token 获取失败，请执行 gh auth login', 'ERROR')
            return False
        token = tok_proc.stdout.strip()
        git_rc, git_out, _ = run_git(['remote', 'get-url', 'origin'])
        if git_rc != 0 or not git_out.strip():
            self._log('❌ 未配置 origin，无法代理推送', 'ERROR')
            return False
        origin_url = git_out.strip()
        m = re.match(r'(https?://)?(?:[^@/]+@)?(ghproxy\.net)/?(https?://.*?)(\.git)?$', origin_url)
        if m and m.group(3):
            gh_url = m.group(3).rstrip('.git')
        else:
            gh_url = origin_url
        mm = re.search(r'github\.com/([^/]+)/', gh_url)
        user = mm.group(1) if mm else 'chleeken'
        proxied = 'https://{}:{}@ghproxy.net/{}'.format(user, token, gh_url.rstrip('.git'))
        self._log('$ git push (via ghproxy.net)', 'INFO')
        proc = subprocess.run(
            ['git', 'push', proxied, branch_name],
            cwd=PROGRAM_DIR,
            stdout=subprocess.PIPE, stderr=subprocess.PIPE,
            shell=False, text=True, encoding='utf-8', errors='replace')
        out2 = (proc.stdout or '') + (proc.stderr or '')
        for line in out2.splitlines()[:20]:
            if 'password' in line.lower() or 'token' in line.lower() or token in line:
                continue
            self._log(line, 'WARN' if proc.returncode != 0 else 'INFO')
        if proc.returncode == 0:
            self._log('✅ 代理推送成功 (via ghproxy.net)', 'SUCCESS')
            return True
        self._log('❌ 代理推送失败', 'ERROR')
        return False

    def _cmd_push_async(self):
        """推送前先检测更新：behind>0 提示/pull --rebase；ahead=0 behind=0 提醒无需重复提交。
        国内直连 GitHub 443 失败时，自动 fallback 到 ghproxy.net + gh token 代理推送。
        """
        branch = self.cfg.get('default_branch', 'main')
        rc, out, _ = run_git(['symbolic-ref', '--short', 'HEAD'])
        if rc == 0 and out.strip():
        ...
        def _doit():
            ...
            if rc3 == 0:
                ...
                if not ok:
                    self._log('⚠️ 直连 fetch 失败，尝试 ghproxy.net 代理...', 'WARN')
                    direct_fail = True
                elif first_push:
            ...
            # 推送：先试直连，失败再 fallback 代理
            rc_push, _, err = run_git(['push', '-u', 'origin', branch])
            if rc_push == 0:
                self._log('✅ 直连 push 成功', 'SUCCESS')
                return
            self._log('⚠️ 直连 push 失败 (exit={})，尝试 ghproxy.net 代理...'.format(rc_push), 'WARN')
            self._push_via_proxy(branch)
